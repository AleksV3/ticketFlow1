...
public class FieldGrantAdminService {
 private final SubtypeFieldDefinitionRepository fields; private final SubtypeFieldRoleGrantRepository grants; private final RoleRepository roles;
 public FieldGrantAdminService(SubtypeFieldDefinitionRepository fields,SubtypeFieldRoleGrantRepository grants,RoleRepository roles){this.fields=fields;this.grants=grants;this.roles=roles;}
 @Transactional public List<SubtypeFieldRoleGrant> replace(AuthPrincipal p,Long fieldId,Collection<Long> view,Collection<Long> edit,Collection<Long> create){
  ...
  grants.deleteAll(grants.findByFieldId(fieldId)); Map<Long,Role> map=new HashMap<>();rs.forEach(r->map.put(r.getId(),r)); List<SubtypeFieldRoleGrant> out=new ArrayList<>(); add(out,f,map,view,FieldGrantOperation.VIEW);add(out,f,map,edit,FieldGrantOperation.EDIT);add(out,f,map,create,FieldGrantOperation.CREATE);return grants.saveAll(out);
 }
