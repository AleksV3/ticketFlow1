...
public class TicketConfigurationService {
    ...
    private void requireManage(AuthPrincipal p){if(p.party()!=Responsibility.TICKETFLOW1||(!p.hasPermission("TYPE_MANAGE")&&!p.hasPermission("WORKFLOW_MANAGE")&&!p.hasPermission("ROLE_MANAGE")))throw ApiException.forbidden("TYPE_MANAGE, WORKFLOW_MANAGE, or ROLE_MANAGE permission is required.");}
