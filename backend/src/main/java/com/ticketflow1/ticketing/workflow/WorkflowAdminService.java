...
public class WorkflowAdminService {
    ...
    private Organization organization(AuthPrincipal p,Long requested){Long id=scope(p,requested);if(id==null)throw ApiException.validation("organizationId is required.");return organizations.findById(id).orElseThrow(()->ApiException.notFound("Organization not found: "+id));}
