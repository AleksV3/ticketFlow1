...
public class DeveloperTeamService {
    private final DeveloperTeamRepository teams; private final AppUserRepository users; private final TicketRepository tickets;
    public DeveloperTeamService(DeveloperTeamRepository teams,AppUserRepository users,TicketRepository tickets){this.teams=teams;this.users=users;this.tickets=tickets;}

    @Transactional(readOnly=true) public List<TeamDtos.TeamResponse> list(AuthPrincipal p){internal(p);return teams.findAllByOrderByNameAsc().stream().map(t->TeamDtos.TeamResponse.from(t,canEdit(t,p))).toList();}
    @Transactional(readOnly=true) public TeamDtos.Options options(AuthPrincipal p){internal(p);var people=users.findByActiveTrueAndPartyOrderByDisplayNameAsc(Responsibility.TICKETFLOW1).stream().map(u->new TeamDtos.Person(u.getId(),u.getDisplayName())).toList();var refs=tickets.findAll().stream().map(t->new TeamDtos.TicketRef(t.getTicketKey(),t.getTitle(),t.getCurrentState().getKey())).sorted(Comparator.comparing(TeamDtos.TicketRef::ticketKey)).toList();return new TeamDtos.Options(people,refs);}
    @Transactional public TeamDtos.TeamResponse create(TeamDtos.SaveTeamRequest r,AuthPrincipal p){internal(p);AppUser actor=user(p.userId());AppUser leader=p.hasPermission("USER_MANAGE")&&r.leaderId()!=null?internalUser(r.leaderId()):actor;DeveloperTeam team=new DeveloperTeam(name(r.name()),text(r.description()),leader,actor);apply(team,r,leader);return TeamDtos.TeamResponse.from(teams.save(team),true);}
    @Transactional public TeamDtos.TeamResponse update(Long id,TeamDtos.SaveTeamRequest r,AuthPrincipal p){internal(p);DeveloperTeam team=teams.findById(id).orElseThrow(()->ApiException.notFound("Team not found."));if(!canEdit(team,p))throw ApiException.forbidden("Only an administrator or this team's leader can edit it.");AppUser leader=r.leaderId()==null?team.getLeader():internalUser(r.leaderId());apply(team,r,leader);return TeamDtos.TeamResponse.from(teams.save(team),true);}
