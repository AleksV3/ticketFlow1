...
describe("permission-driven UI", () => {
  it("only returns navigation authorized by the API permissions", () => {
    expect(permittedLinks(["TICKET_READ", "USER_MANAGE"]).map(link => link.label))
      .toEqual(["Dashboard", "Tickets", "Organizations", "Users"]);
  });

  it.each([
    { role: "Admin", permissions: ["TICKET_READ", "TICKET_CREATE", "USER_MANAGE", "ROLE_MANAGE", "WORKFLOW_MANAGE"], links: 7 },
    { role: "TicketFlow1 User", permissions: ["TICKET_READ", "TICKET_CREATE"], links: 3 },
    { role: "TicketFlow1 Manager", permissions: ["TICKET_READ", "TICKET_CREATE", "USER_MANAGE"], links: 5 },
    { role: "Client User", permissions: ["TICKET_READ", "TICKET_CREATE"], links: 3 },
    { role: "Client Approver", permissions: ["TICKET_READ", "TICKET_CREATE", "ROLE_MANAGE"], links: 4 },
