...
export function AppShell({ children, require }: { children: (user: CurrentUser) => ReactNode; require?: string }) {
  const router = useRouter(); const pathname = usePathname(); const [user,setUser]=useState<CurrentUser|null>(null); const [unreadNotifications,setUnreadNotifications]=useState(0); const [toast,setToast]=useState<{title:string;message:string;ticketKey:string|null}|null>(null); const previousUnread=useRef<number|null>(null); const [error,setError]=useState("");
  const refreshUnread = useCallback(async () => { try { const count=await get<number>("/notifications/unread-count"); const previous=previousUnread.current; setUnreadNotifications(count); previousUnread.current=count; if(previous!==null && count>previous){ const latest=await get<Array<{title:string;message:string;read:boolean;ticketKey:string|null}>>("/notifications"); const item=latest.find(notification=>!notification.read); if(item){setToast({title:item.title,message:item.message,ticketKey:item.ticketKey}); window.setTimeout(()=>setToast(null),7000); } } } catch { /* session guard handles authentication */ } }, []);
  ...
  return <div className="app-frame"><header className="app-header"><div className="mx-auto flex max-w-7xl flex-wrap items-center gap-5 px-5 py-4"><Link href="/dashboard" className="brand-mark text-lg">TicketFlow1</Link><nav aria-label="Main navigation" className="flex min-w-0 flex-1 flex-wrap gap-1">{permittedLinks(user.permissions,user.party).map(l=>{const active=pathname===l.href || (l.href!=="/dashboard" && pathname.startsWith(`${l.href}/`)); return <Link aria-current={active?"page":undefined} className={`nav-link ${active?"nav-link-active":""}`} href={l.href} key={l.href}>{l.label}{l.href==="/notifications" && unreadNotifications>0 ? <span className="notification-dot" aria-label={`${unreadNotifications} unread notifications`} title={`${unreadNotifications} unread notifications`} /> : null}</Link>})}</nav><span className="rounded-full border border-slate-200/20 px-3 py-2 text-sm text-slate-500">{user.displayName}</span><ThemeController/><button className="btn-secondary" onClick={async()=>{await logout();router.replace("/login");}}>Sign out</button></div></header><main className="page-shell mx-auto max-w-7xl p-5 sm:p-8">{children(user)}</main>{toast?<aside className={`notification-toast ${toast.ticketKey?"notification-toast-clickable":""}`} role="status">{toast.ticketKey?<Link href={`/tickets/${toast.ticketKey}`} onClick={()=>setToast(null)}><p className="eyebrow">New notification</p><strong>{toast.title}</strong><p>{toast.message}</p></Link>:<><p className="eyebrow">New notification</p><strong>{toast.title}</strong><p>{toast.message}</p></>}<button className="notification-toast-close" aria-label="Dismiss notification" onClick={()=>setToast(null)}>×</button></aside>:null}</div>;
}
