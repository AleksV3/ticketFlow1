...
export const NAV_LINKS=[{href:"/dashboard",label:"Dashboard",permission:"TICKET_READ"},{href:"/tickets",label:"Tickets",permission:"TICKET_READ"},{href:"/tickets/new",label:"New ticket",permission:"TICKET_CREATE"},{href:"/admin/users",label:"Users",permission:"USER_MANAGE"},{href:"/admin/roles",label:"Roles",permission:"ROLE_MANAGE"},{href:"/admin/workflows",label:"Workflows",permission:"WORKFLOW_MANAGE"}];
export const permittedLinks=(permissions:string[])=>NAV_LINKS.filter(link=>permissions.includes(link.permission));
...
export function AppShell({ children, require }: { children: (user: CurrentUser) => ReactNode; require?: string }) {
  ...
  return <div className="min-h-screen bg-slate-100"><header className="border-b bg-white"><div className="mx-auto flex max-w-7xl flex-wrap items-center gap-5 px-5 py-4"><Link href="/dashboard" className="text-lg font-bold">TicketFlow1</Link><nav aria-label="Main navigation" className="flex flex-1 flex-wrap gap-1">{permittedLinks(user.permissions).map(l=><Link className="rounded-lg px-3 py-2 text-sm hover:bg-slate-100" href={l.href} key={l.href}>{l.label}</Link>)}</nav><span className="text-sm text-slate-600">{user.displayName}</span><button className="btn-secondary" onClick={async()=>{await logout();router.replace("/login");}}>Sign out</button></div></header><main className="mx-auto max-w-7xl p-5 sm:p-8">{children(user)}</main></div>;
}
