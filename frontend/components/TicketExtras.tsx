...
function AttachmentPreview({ preview, download }: { preview: { attachment: Attachment; url?: string; text?: string }; download: () => void }) {
  ...
  return <section className="attachment-inline-preview" aria-label={`Preview ${attachment.fileName}`}><header><div className="min-w-0"><strong className="block truncate text-sm">{attachment.fileName}</strong><span className="text-xs text-slate-500">Preview</span></div><button className="btn-secondary px-2 py-1 text-xs" onClick={download}>Download</button></header><div className="attachment-inline-body">{text !== undefined ? <pre>{text}</pre> : image && url ? <img src={url} alt={attachment.fileName}/> : pdf && url ? <iframe src={url} title={attachment.fileName}/> : audio && url ? <audio controls src={url}/> : video && url ? <video controls src={url}/> : <div className="p-4 text-center text-sm text-slate-500"><p>This file type cannot be previewed safely.</p><button className="btn-primary mt-3" onClick={download}>Download and open</button></div>}</div></section>;
}
