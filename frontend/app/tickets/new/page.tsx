...
function TicketForm({ user }: { user: CurrentUser }) {
  ...
  const canAssign = user.party === "TICKETFLOW1" && user.permissions.includes("TICKET_ASSIGN");
  ...
  const canAssignForOrganization = canAssign && selectedOrganization.toLowerCase() === "ticketflow1 internal";
  ...
  useEffect(() => {
    if (!canAssignForOrganization) {
      setLeadId("");
      setDeveloperIds([]);
      setTeamIds([]);
    }
  }, [canAssignForOrganization]);
  useEffect(() => {
  ...
  async function submit(event: FormEvent) {
    ...
    try {
      const ticket = await post<TicketDetail>("/tickets", {
        ...
        ticketLeadId: canAssignForOrganization && leadId ? Number(leadId) : null,
        developerIds: canAssignForOrganization ? developerIds : null,
        teamIds: canAssignForOrganization ? teamIds : null,
