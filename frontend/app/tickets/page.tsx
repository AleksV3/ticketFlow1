...
function Tickets() {
  ...
  return <div><div className="flex items-end justify-between"><div><h1 className="text-3xl font-bold">Tickets</h1><p className="mt-1 text-slate-600">Search and filter the current ticket scope.</p></div><Link className="btn-primary" href="/tickets/new">New ticket</Link></div>
    <div className="card mt-6 grid gap-3 sm:grid-cols-2 lg:grid-cols-6"><label className="sm:col-span-2 lg:col-span-2"><span className="text-sm">Search tickets and people</span><input className="field mt-1" defaultValue={query.get("q") ?? ""} placeholder="Key, title, organization, lead or developer…" onBlur={e => set("q", e.target.value)} onKeyDown={e => { if (e.key === "Enter") set("q", e.currentTarget.value); }} /></label><Filter label="Lifecycle" value={query.get("lifecycle") ?? ""} onChange={v => set("lifecycle", v)} options={["active", "closed"]} /><Filter label="Type" value={query.get("type") ?? ""} onChange={v => set("type", v)} options={["CHANGE_REQUEST", "TASK", "DEFECT"]} /><Filter label="Severity" value={query.get("severity") ?? ""} onChange={v => set("severity", v)} options={["SEV_1", "SEV_2", "SEV_3", "SEV_4"]} /><Filter label="SLA" value={query.get("slaStatus") ?? ""} onChange={v => set("slaStatus", v)} options={["OK", "DUE_SOON", "BREACHED", "NOT_APPLICABLE"]} /><Filter label="Assigned" value={query.get("assignedTo") ?? ""} onChange={v => set("assignedTo", v)} options={["me", "unassigned"]} /></div>
    {error ? <div className="card mt-5 text-red-700">{error}</div> : !data ? <div className="card mt-5">Loading tickets…</div> : data.items.length === 0 ? <div className="card mt-5">No tickets match these filters.</div> : <><div className="mt-5 overflow-x-auto rounded-xl border bg-white"><table className="w-full min-w-[1100px] text-left text-sm"><thead className="bg-slate-50"><tr>{["Key / ticket", "Type", "Severity", "Status", "Team lead", "Developers", "SLA", "Updated"].map(h => <th className="p-3" key={h}>{h}</th>)}</tr></thead><tbody>{data.items.map(t => <tr className="border-t" key={t.ticketKey}><td className="max-w-72 p-3"><Link className="font-semibold hover:underline" href={`/tickets/${t.ticketKey}`}>{t.ticketKey}</Link><span className="mt-1 block truncate text-xs text-slate-500" title={t.title}>{t.title}</span></td><td className="p-3">{t.type.replaceAll("_", " ")}</td><td className="p-3"><Severity value={t.severity} /></td><td className="p-3"><StatusBadge value={t.status} /></td><td className="p-3"><Person value={t.ticketLeadName} empty="Unassigned" /></td><td className="max-w-60 p-3"><Person value={t.developerNames?.join(", ")} empty="No developers" /></td><td className="p-3"><SlaBadge value={t.slaStatus} /></td><td className="p-3">{new Date(t.updatedAt).toLocaleDateString()}</td></tr>)}</tbody></table></div><Pagination page={data.page} totalPages={data.totalPages} onPage={p => set("page", String(p), false)} /></>}
  </div>;
}
