...
function Tickets() {
  ...
  type Draft = { q: string; lifecycle: string; type: string; subtype: string; parentTicketKey: string; severity: string; slaStatus: string; assignedTo: string };
  const draftFromQuery = (): Draft => ({ q: query.get("q") ?? "", lifecycle: query.get("lifecycle") ?? "", type: query.get("type") ?? "", subtype: query.get("subtype") ?? "", parentTicketKey: query.get("parentTicketKey") ?? "", severity: query.get("severity") ?? "", slaStatus: query.get("slaStatus") ?? "", assignedTo: query.get("assignedTo") ?? "" });
  ...
  function clear() { const empty: Draft = { q: "", lifecycle: "", type: "", subtype: "", parentTicketKey: "", severity: "", slaStatus: "", assignedTo: "" }; setDraft(empty); router.push("/tickets?page=0"); }
