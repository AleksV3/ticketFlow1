...
function Detail({ currentUserId, canEdit, canAssign, admin, internal }: { currentUserId: number; canEdit: boolean; canAssign: boolean; admin: boolean; internal: boolean }) {
  ...
  const [ticket, setTicket] = useState<TicketDetail | null>(null), [history, setHistory] = useState<History[]>([]), [audit, setAudit] = useState<Audit[]>([]), [following, setFollowing] = useState(false), [followSaving, setFollowSaving] = useState(false), [error, setError] = useState(""), [editing, setEditing] = useState(false), [lockExpiresAt, setLockExpiresAt] = useState<string | null>(null), [typeDraft, setTypeDraft] = useState<TypeDraft | null>(null);
  ...
  async function toggleEdit() {
    if (ticket && !(canEdit || admin || ticket.businessOwner.id === currentUserId)) {
      window.dispatchEvent(new CustomEvent("ticketflow:toast", { detail: { title: "Editing unavailable", message: "You can edit this ticket only if you created it or have ticket edit permission." } }));
      return;
    }
    if (editing) {
      try { await api(`/tickets/${ticketKey}/edit-lock`, { method: "DELETE" }); } catch { /* lock expiry is harmless */ }
      setLockExpiresAt(null); setEditing(false); return;
    }
    try { const lock = await post<{ expiresAt: string }>(`/tickets/${ticketKey}/edit-lock`); setLockExpiresAt(lock.expiresAt); }
    catch (error) { window.dispatchEvent(new CustomEvent("ticketflow:toast", { detail: { title: "Editing unavailable", message: error instanceof Error ? error.message : "Someone is editing this ticket, so you cannot edit it right now." } })); return; }
    setEditing(true);
  }
  ...
  return <div className="space-y-4">
    <div className="grid items-start gap-4 xl:grid-cols-[minmax(0,1fr)_300px]">
      <main className="min-w-0 overflow-hidden rounded-2xl border border-blue-500/70 bg-slate-900/60 shadow-xl">
        <header className="flex flex-wrap items-center justify-between gap-3 border-b border-slate-700 bg-slate-950/70 px-5 py-4"><div className="flex min-w-0 items-center gap-3"><span className="rounded-lg bg-blue-600 px-3 py-1 text-sm font-bold text-white">{ticket.ticketKey}</span><span className="truncate text-lg font-semibold" title={ticket.title}>{ticket.title}</span><StatusBadge value={ticket.status}/>{ticket.sla ? <SlaBadge value={ticket.sla.status}/> : null}</div><div className="flex flex-wrap gap-2"><button className={following?"btn-primary":"btn-secondary"} disabled={followSaving} onClick={() => void toggleFollow()}>{followSaving?"Saving…":following?"Following":"Follow ticket"}</button>{(canEdit || admin || ticket.businessOwner.id === currentUserId) ? <button className="btn-secondary" onClick={() => void toggleEdit()}>{editing ? "Edit open" : "Edit ticket"}</button> : null}{editing ? <button className="btn-secondary" onClick={() => void toggleEdit()}>Cancel</button> : null}</div></header>
        {editing ? <div className="border-b border-blue-500/40 bg-slate-950/30 p-4">{(admin || ticket.businessOwner.id === currentUserId) && ticket.processMap.states.find(state => state.key === ticket.status)?.isInitial ? <TicketTypeEditor ticket={ticket} done={load}/> : null}{(admin || ticket.businessOwner.id === currentUserId) && !ticket.processMap.states.find(state => state.key === ticket.status)?.isInitial ? <p className="mb-4 rounded-lg border border-amber-400/40 bg-amber-400/10 p-3 text-sm text-amber-200">Ticket type is locked because the workflow has already started.</p> : null}<Edit ticket={ticket} canEdit={canEdit || admin || ticket.businessOwner.id === currentUserId} canAssign={canAssign} done={async () => { setEditing(false); setTypeDraft(null); setLockExpiresAt(null); await load(); }}/>{lockExpiresAt ? <p className="mt-2 text-xs text-amber-300">Edit lock expires {formatDate(lockExpiresAt)}. Unsaved changes will be rolled back when it expires.</p> : null}</div> : null}
        <div className="space-y-4 p-4"><section className="rounded-xl border border-slate-700 bg-slate-900/70 p-4"><div className="grid gap-3 sm:grid-cols-3 lg:grid-cols-5"><Row k="Type" v={ticket.type}/><Row k="Priority" v={ticket.priority}/><Row k="Responsibility" v={ticket.currentResponsibility}/><Row k="Organization" v={ticket.organization.name}/><Row k="Owner" v={ticket.businessOwner.displayName}/></div><div className="mt-4 border-t border-slate-700 pt-4"><p className="text-[10px] uppercase tracking-wider text-slate-500">Description</p><p className="mt-2 text-sm leading-6 text-slate-300 whitespace-pre-wrap">{ticket.description}</p></div></section>
          <TeamPanel ticket={ticket}/><TicketContext ticket={ticket}/>{canAssign ? <details className="rounded-xl border border-slate-700 p-3"><summary className="cursor-pointer font-bold">Edit assignment</summary><div className="mt-4"><Edit ticket={ticket} canEdit={false} canAssign done={load}/></div></details> : null}</div>
    ...
    {(ticket.workflowCommands?.length || ticket.proposalCommands?.length || ticket.latestProposal) ? <details className="card" open><summary className="cursor-pointer list-none"><p className="eyebrow">Decisions and proposals</p><h2 className="text-sm font-bold">Approvals and change proposals</h2><p className="mt-1 text-xs text-slate-500">Review protected workflow decisions and related change proposals in one place.</p></summary><div className="mt-4 space-y-4"><WorkflowDecisionPanel ticketKey={ticketKey} commands={ticket.workflowCommands ?? []} onDone={load}/><ProposalActions ticketKey={ticketKey} proposal={ticket.latestProposal} commands={ticket.proposalCommands ?? []} onDone={load}/></div></details> : null}
...
function TicketSidebar({ ticket, audit, onTransition }: { ticket: TicketDetail; audit: Audit[]; onTransition: (status: string) => Promise<void> }) {
  ...
  async function move(status: string) {
    ...
    try { await onTransition(status); }
      ...
      const message = error instanceof Error ? error.message : "Could not advance the workflow.";
      setTransitionError(message.toLowerCase().includes("being edited") || message.toLowerCase().includes("locked")
      ? (window.dispatchEvent(new CustomEvent("ticketflow:toast", { detail: { title: "Editing unavailable", message: "Someone is editing this ticket, so this action cannot be completed right now." } })), "")
        : message);
    }
  }
  return <aside className="card sticky top-4 space-y-5 py-4">
    ...
    <div className="border-t border-slate-700 pt-4"><p className="eyebrow">Next move</p><p className="mt-1 text-xs text-slate-400">Available actions from this workflow state.</p><div className="mt-3 grid gap-2"><TransitionButtons allowedTransitions={ticket.allowedTransitions} onTransition={move}/></div>{transitionError ? <p className="mt-3 rounded-lg border border-amber-400/50 bg-amber-400/10 p-3 text-sm text-amber-200" role="alert">{transitionError}</p> : null}{!ticket.allowedTransitions.length ? <p className="mt-2 text-sm text-slate-500">No status transitions available.</p> : null}</div>
    <Link className="btn-secondary block w-full text-center" href="#audit-history" onClick={event => { event.preventDefault(); const target = document.getElementById("audit-history") as HTMLDetailsElement | null; if (target) { target.open = true; target.scrollIntoView({ behavior: "smooth", block: "start" }); } }}>View audit history</Link>
  </aside>;
}
...
type TicketTypeOption={id:number;key:string;name:string};
type EditField={id:number;key:string;label:string;helpText:string|null;fieldKind:string;required:boolean;options:{id:number;key:string;label:string}[]};
type EditSubtype={id:number;key:string;name:string;description:string|null;fields:EditField[]};
type EditCreationForm={id:number;key:string;name:string;subtypes:EditSubtype[]};
type TypeDraft={type:string;subtypeId:string;dynamicValues:Record<string,unknown>};
function TicketTypeEditor({ ticket, done, onDraft, hideSave=false }: { ticket: TicketDetail; done: () => Promise<void>; onDraft?: (draft:TypeDraft)=>void; hideSave?: boolean }) {
  const [types, setTypes] = useState<TicketTypeOption[]>([]), [value, setValue] = useState(ticket.type), [form, setForm] = useState<EditCreationForm|null>(null), [subtypeId, setSubtypeId] = useState(ticket.subtypeId ? String(ticket.subtypeId) : ""), [values, setValues] = useState<Record<string,unknown>>(ticket.dynamicValues ?? {}), [saving, setSaving] = useState(false), [message, setMessage] = useState("");
  useEffect(() => { void get<TicketTypeOption[]>(`/reference/ticket-types?organizationId=${ticket.organization.id}`).then(setTypes).catch(error => setMessage(error instanceof Error ? error.message : "Could not load ticket types.")); }, [ticket.organization.id]);
  useEffect(() => { const selected = types.find(type => type.key === value); if (!selected) return; void get<EditCreationForm>(`/reference/ticket-types/${selected.id}/creation-form`).then(next => { setForm(next); setSubtypeId(selected.key === ticket.type && ticket.subtypeId ? String(ticket.subtypeId) : (next.subtypes[0] ? String(next.subtypes[0].id) : "")); setValues(selected.key === ticket.type ? (ticket.dynamicValues ?? {}) : {}); }).catch(error => setMessage(error instanceof Error ? error.message : "Could not load subtype fields.")); }, [types, value, ticket.type, ticket.subtypeId, ticket.dynamicValues]);
  const subtype = form?.subtypes.find(item => String(item.id) === subtypeId) ?? null;
  function publish(next:TypeDraft){onDraft?.(next);}
  function setField(key:string, fieldValue:unknown){setValues(current => { const next={...current,[key]:fieldValue}; publish({type:value,subtypeId,dynamicValues:next}); return next; });}
  async function save() { setSaving(true); setMessage(""); try { await patch(`/tickets/${ticket.ticketKey}`, { ticketTypeKey: value, ...(subtypeId ? { subtypeId: Number(subtypeId), dynamicValues: compactEditValues(values, subtype?.fields ?? []) } : {}) }); await done(); setMessage("Ticket type and subtype saved."); } catch (error) { setMessage(error instanceof Error ? error.message : "Could not save ticket type."); } finally { setSaving(false); } }
  return <section className="card mb-4 grid gap-3"><label className="block font-semibold">Ticket type<select className="field mt-1" value={value} onChange={event => { const next=event.target.value; setValue(next); setForm(null); setSubtypeId(""); setValues({}); publish({type:next,subtypeId:"",dynamicValues:{}}); }}>{types.map(type => <option value={type.key} key={type.id}>{type.name} ({type.key})</option>)}</select></label>{form?.subtypes.length ? <label className="block font-semibold">Subtype<select className="field mt-1" value={subtypeId} onChange={event => { const next=event.target.value; const nextValues=value === ticket.type && next === String(ticket.subtypeId) ? (ticket.dynamicValues ?? {}) : {}; setSubtypeId(next); setValues(nextValues); publish({type:value,subtypeId:next,dynamicValues:nextValues}); }}>{form.subtypes.map(item => <option value={item.id} key={item.id}>{item.name} ({item.key})</option>)}</select>{subtype?.description ? <small className="text-slate-500">{subtype.description}</small> : null}</label> : null}{subtype ? <div className="grid gap-3 sm:grid-cols-2">{subtype.fields.map(field => <EditDynamicField key={field.id} field={field} value={values[field.key]} setValue={next => setField(field.key,next)}/>)}</div> : null}{hideSave ? null : <div><button type="button" className="btn-primary" disabled={saving || (value === ticket.type && String(ticket.subtypeId ?? "") === subtypeId && JSON.stringify(values) === JSON.stringify(ticket.dynamicValues ?? {}))} onClick={() => void save()}>{saving ? "Saving…" : "Save type and subtype"}</button>{message ? <span className="ml-3 text-sm" role="status">{message}</span> : null}</div>}</section>;
}
function compactEditValues(values:Record<string,unknown>, fields:EditField[]){const allowed=new Set(fields.map(field=>field.key));return Object.fromEntries(Object.entries(values).filter(([key,value])=>allowed.has(key)&&value!==""&&value!==null&&value!==undefined&&(!Array.isArray(value)||value.length)));}
function EditDynamicField({field,value,setValue}:{field:EditField;value:unknown;setValue:(value:unknown)=>void}){const label=<span>{field.label}</span>;if(field.fieldKind==="LONG_TEXT")return <label className="block sm:col-span-2">{label}<textarea className="field mt-1 min-h-24" required={field.required} value={String(value??"")} onChange={event=>setValue(event.target.value)}/>{field.helpText?<small className="text-slate-500">{field.helpText}</small>:null}</label>;if(field.fieldKind==="BOOLEAN")return <label className="permission-option"><input type="checkbox" checked={Boolean(value)} onChange={event=>setValue(event.target.checked)}/><span><strong>{field.label}</strong>{field.helpText?<small>{field.helpText}</small>:null}</span></label>;if(field.fieldKind==="SINGLE_SELECT")return <label className="block">{label}<select className="field mt-1" required={field.required} value={String(value??"")} onChange={event=>setValue(event.target.value)}><option value="">Select...</option>{field.options.map(option=><option value={option.key} key={option.id}>{option.label}</option>)}</select></label>;const inputType=field.fieldKind==="DATE"?"date":field.fieldKind==="INTEGER"||field.fieldKind==="DECIMAL"?"number":"text";return <label className="block">{label}<input className="field mt-1" type={inputType} required={field.required} value={String(value??"")} onChange={event=>setValue(inputType==="number"?Number(event.target.value):event.target.value)}/>{field.helpText?<small className="text-slate-500">{field.helpText}</small>:null}</label>;}
function Edit({ ticket, canEdit, canAssign, done }: { ticket: TicketDetail; canEdit:boolean; canAssign:boolean; done: () => Promise<void> }) { const [title, setTitle] = useState(ticket.title), [description, setDescription] = useState(ticket.description), [priority, setPriority] = useState(ticket.priority), [severity, setSeverity] = useState(ticket.severity ?? "SEV_3"), [people,setPeople]=useState<Person[]>([]),[teams,setTeams]=useState<TeamOption[]>([]),[leadId,setLeadId]=useState(ticket.ticketLead?String(ticket.ticketLead.id):""),[developerIds,setDeveloperIds]=useState<number[]>(ticket.developers?.map(item=>item.id)??[]),[teamIds,setTeamIds]=useState<number[]>(ticket.teams?.map(item=>item.id)??[]), [saving, setSaving] = useState(false), [message,setMessage]=useState(""); useEffect(()=>{if(canAssign){void get<Person[]>("/reference/ticket-leads").then(setPeople).catch(error=>setMessage(error instanceof Error?error.message:"Could not load assignable people."));void get<TeamOption[]>("/teams").then(setTeams).catch(error=>setMessage(error instanceof Error?error.message:"Could not load teams."))}},[canAssign]); function toggle(id:number){setDeveloperIds(values=>values.includes(id)?values.filter(value=>value!==id):[...values,id])} function toggleTeam(id:number){setTeamIds(values=>values.includes(id)?values.filter(value=>value!==id):[...values,id])} async function submit(event: FormEvent) { event.preventDefault(); setSaving(true); setMessage(""); try { await patch(`/tickets/${ticket.ticketKey}`, { title, description, ...(canEdit?{priority, ...(ticket.severity ? {severity} : {})}:{}), ...(canAssign?{...(leadId?{ticketLeadId:Number(leadId)}:{}),developerIds,teamIds}: {}) }); await done(); setMessage("Ticket saved successfully."); } catch(error) { setMessage(error instanceof Error?error.message:"Could not save the ticket."); } finally { setSaving(false); } } return <form className="card grid gap-4" onSubmit={submit}><div className="eyebrow">{canEdit ? "Full ticket edit" : "Basic ticket edit"}</div><label>Title<input className="field mt-1" value={title} onChange={event => setTitle(event.target.value)}/></label><label>Description<textarea className="field mt-1 min-h-24" value={description} onChange={event => setDescription(event.target.value)}/></label>{canEdit ? <div className="grid gap-3 sm:grid-cols-2"><label>Priority<select className="field mt-1" value={priority} onChange={event => setPriority(event.target.value)}>{["LOW","MEDIUM","HIGH","CRITICAL"].map(value=><option value={value} key={value}>{value}</option>)}</select></label>{ticket.severity ? <label>Severity<select className="field mt-1" value={severity} onChange={event => setSeverity(event.target.value)}>{["SEV_1","SEV_2","SEV_3","SEV_4"].map(value=><option value={value} key={value}>{value}</option>)}</select></label> : null}</div> : <p className="text-xs text-slate-500">You can edit only the title and description.</p>}{canAssign?<fieldset className="space-y-3"><legend className="font-bold">Ticket team</legend><label className="block">Ticket lead <span className="text-xs font-normal text-slate-500">(optional)</span><select className="field mt-1" value={leadId} onChange={event=>setLeadId(event.target.value)}><option value="">No team lead selected</option>{people.map(person=><option key={person.id} value={person.id}>{person.name}</option>)}</select></label><div><span className="text-sm font-medium">Developer teams</span><div className="mt-2 grid gap-2 sm:grid-cols-2 lg:grid-cols-3">{teams.map(team=><label className={`permission-option ${teamIds.includes(team.id)?"permission-option-selected":""}`} key={team.id}><input type="checkbox" checked={teamIds.includes(team.id)} onChange={()=>toggleTeam(team.id)}/><span><strong>{team.name}</strong><small>{teamIds.includes(team.id)?"Assigned team":"Assign team"}</small></span></label>)}</div></div><div><div className="flex items-center justify-between gap-3"><span className="text-sm font-medium">Developers</span><span className="text-xs text-slate-500">{developerIds.length} selected</span></div><div className="mt-2 grid gap-2 sm:grid-cols-2 lg:grid-cols-3">{people.map(person=><label className={`permission-option ${developerIds.includes(person.id)?"permission-option-selected":""}`} key={person.id}><input type="checkbox" checked={developerIds.includes(person.id)} onChange={()=>toggle(person.id)}/><span><strong>{person.name}</strong><small>{developerIds.includes(person.id)?"Assigned developer":"Select as developer"}</small></span></label>)}</div></div></fieldset>:null}<div className="flex flex-wrap items-center gap-3"><button className="btn-primary" disabled={saving}>{saving ? "Saving…" : "Save changes"}</button>{message?<span className={`text-sm ${message.includes("successfully")?"text-emerald-400":"text-red-400"}`} role="status">{message}</span>:null}</div></form>; }
